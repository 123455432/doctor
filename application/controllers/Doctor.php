...
class Doctor extends CI_Controller {
	...
	public function Profile(){				
		...
		$template['specialtydetails'] =$this->Doctor_Model->get_specialties();
		$template['view_galpic'] =$this->Doctor_Model->get_singlegalpic($id);		
		$template['view_doctor'] =$this->Doctor_Model->get_singledoctor($id);
		if(isset($template['view_doctor'])&!empty($template['view_doctor'])){
			$template['review_doctor'] =$this->Doctor_Model->get_singlereview($id);		
			...
			if($result->latitude != ""){		
				if($result->longitude != ""){       
					...
					$marker['infowindow_content']= '<img  class="img-mp-section" src= '.$result->display_image.' >'."".'<h2>'.'Dr.'.$result->doctor_firstname.'</h2>';				
					$this->googlemaps->add_marker($marker);
					}
				}	
					$template['map'] = $this->googlemaps->create_map();	
					$template['page'] = "doctor_profile";
					$template['page_title'] = "profile Page";
					$template['data'] = "profile page";
					$this->load->view('template', $template);	
			}
			else{
