...
class Welcome extends CI_Controller {
	...
	public function index()
	{
				...
				/********************role play (super persons) ****************************/
				if ($this->session->userdata('super_person') == 1)
					{
						/********************get id from super person1(doctor)****************************/
						$id=$this->session->userdata['frontend_logged_in']['id'];				
						/******************different ways to submit the data  ******************************/ 
						/***********************my details*************************/
						...
						if ($this->upload->do_upload('display_image'))
							{
								...
								$fname = $file_data['file_name'];
								$config['upload_path'] = 'uploads/';
								$data['display_image']=base_url().$config['upload_path'].$fname;							
							}
						}						
								...
								$template['doctor_pictures'] =$this->Welcome_Model->get_singledoctorpictures($id);					
								$template['doctor_schedule'] =$this->Welcome_Model->get_singledoctorschedule($id);			
								$template['doctor_personal'] =$this->Welcome_Model->get_singledoctor($id);		
								$template['degree'] =$this->Welcome_Model->get_degree();
								$template['affilliation'] =$this->Welcome_Model->get_affilliation();
								$template['visitation'] =$this->Welcome_Model->get_visitation();
								$template['states'] =$this->Welcome_Model->get_states();
								$template['cities'] =$this->Welcome_Model->get_cities();
								$template['countries'] =$this->Welcome_Model->get_countries();	
								$template['languages'] =$this->Welcome_Model->get_languages();
								$template['specialties'] =$this->Welcome_Model->get_specialties();
								$template['insurance'] =$this->Welcome_Model->get_insurance();
								$template['all_package'] = $this->Welcome_Model->get_packages();	
								//for serach slider
								...
								$template['page'] = "Welcome_doctor";
					}
			/*********************super person0 (patient)***************************/
						...
						if ($this->upload->do_upload('patient_display_image'))
							{
								...
								$fname = $file_data['file_name'];
								$config['upload_path'] = 'uploads/';
								$data['patient_display_image']=base_url().$config['upload_path'].$fname;							
							}
						}						
       ...
       if($result) {
        if($result === "notexist") {
						...
						$template['final_patient_history'] =$this->Welcome_Model->get_patient_history_final($id);
						$template['patient_history'] =$this->Welcome_Model->get_patient_history_final($id);
						$template['an_patient_history'] =$this->Welcome_Model->get_an_patient_history($id);
						$new_app_pat = array();
					foreach($template['patient_history'] as $docpatdetail){				
						 $docpatdetail->total = $this->Welcome_Model->get_hospapppat_doctor($docpatdetail->id,$id);
						 $new_app_pat[] = $docpatdetail;
			 }
						 $current_date = date('Y-m-d');
						 $template['an_patientc_history'] =$this->Welcome_Model->get_an_check_patient_history($id,$current_date);
						
						$new_app_pat = array();
			 foreach($template['an_patientc_history'] as $docpdatdetail){				
						 $docpdatdetail->total = $this->Welcome_Model->get_hospapppat_doctor($docpdatdetail->id,$id);
						 $new_app_pat[] = $docpdatdetail;
			 }			
//var_dump($template['an_patientc_history']);exit;			 
						$template['patient_personal'] =$this->Welcome_Model->get_singlepatient($id);
					//for serach slider						
						$template['countriesslider'] =$this->Home_Model->get_countries();	
						$template['languagesslider'] =$this->Home_Model->get_languages();
						$template['specialtiesslider'] =$this->Home_Model->get_specialties();
						$template['insuranceslider'] =$this->Home_Model->get_insurance();
						$template['page'] = "Welcome_patient";
			}
			/*********************super person2 (clinic)***************************/
			elseif ($this->session->userdata('super_person') == 2)
			{
						$id=$this->session->userdata['frontend_logged_in']['id'];
						$template['sub_hospitals'] = $this->Welcome_Model->get_sub_clinic($id);
						$template['app_doc'] = $this->Welcome_Model->get_appclinic_doctor($id);
						$template['hospital_data'] = $this->Welcome_Model->get_singleclinic_hospital($id);
						$new_app = array();
						 foreach($template['app_doc'] as $docdetail){							
			...
			if ($this->form_validation->run() == FALSE)
				...
				if($result){
				  ...
				  //}
							...
							//unset ($data['hospital_affilliations']);				  
			...
			$template['gallery_clinic_pictures'] =$this->Welcome_Model->get_gallery_clinic($id);				
			$template['mydoc_hospital'] =$this->Welcome_Model->get_doc_clinics($id);	
			$template['degree'] =$this->Welcome_Model->get_degree();
				$template['affilliation'] =$this->Welcome_Model->get_affilliation();
				$template['visitation'] =$this->Welcome_Model->get_visitation();
				$template['states'] =$this->Welcome_Model->get_states();
	        ...
	        $template['languages'] =$this->Welcome_Model->get_languages();
	        $template['specialties'] =$this->Welcome_Model->get_specialties();			
			$template['amenities'] =$this->Welcome_Model->get_amenities();		
	        $template['insurance'] =$this->Welcome_Model->get_insurance();	
				
				//for serach slider
				$template['all_package'] = $this->Welcome_Model->get_center_packages();
				$template['countriesslider'] =$this->Home_Model->get_countries();	
				$template['languagesslider'] =$this->Home_Model->get_languages();
				$template['specialtiesslider'] =$this->Home_Model->get_specialties();
				$template['insuranceslider'] =$this->Home_Model->get_insurance();
				$template['page'] = "Welcome_clinic";
			}
			/*********************super person3 (medical)***************************/
			elseif ($this->session->userdata('super_person') == 3)
			{
				$id=$this->session->userdata['frontend_logged_in']['id'];
				$template['sub_hospitals'] = $this->Welcome_Model->get_sub_medical($id);
				$template['app_doc'] = $this->Welcome_Model->get_appmedical_doctor($id);
				$template['hospital_data'] = $this->Welcome_Model->get_singlemedical_hospital($id);
				$new_app = array();
			 foreach($template['app_doc'] as $docdetail){				
		...
		if ($this->form_validation->run() == FALSE)
			...
			if($result) {
				...
				$template['gallery_medical_pictures'] =$this->Welcome_Model->get_gallery_medicals($id);			
				$template['mydoc_hospital'] =$this->Welcome_Model->get_doc_medicals($id);	
				$template['degree'] =$this->Welcome_Model->get_degree();
				$template['affilliation'] =$this->Welcome_Model->get_affilliation();
				$template['visitation'] =$this->Welcome_Model->get_visitation();
				$template['states'] =$this->Welcome_Model->get_states();
				...
				$template['specialtiesslider'] =$this->Home_Model->get_specialties();
				$template['insuranceslider'] =$this->Home_Model->get_insurance();
				$template['page'] = "Welcome_medical";
			}
			/*********************super person4 (hospital)***************************/
			elseif ($this->session->userdata('super_person') == 4)
			{
					$id=$this->session->userdata['frontend_logged_in']['id'];
					$template['sub_hospitals'] = $this->Welcome_Model->get_sub_hospital($id);
					$template['app_doc'] = $this->Welcome_Model->get_app_doctor($id);
					$template['hospital_data'] = $this->Welcome_Model->get_single_hospital($id);
					$new_app = array();
			 foreach($template['app_doc'] as $docdetail){				
		...
		if ($this->form_validation->run() == FALSE)
			...
			if($result) {
				...
				if($result){
					...
					$template['gallery_hospital_pictures'] =$this->Welcome_Model->get_gallery_hospitals($id);			
					$template['mydoc_hospital'] =$this->Welcome_Model->get_doc_hospitals($id);	
					$template['degree'] =$this->Welcome_Model->get_degree();
					$template['affilliation'] =$this->Welcome_Model->get_affilliation();
					$template['visitation'] =$this->Welcome_Model->get_visitation();
					$template['states'] =$this->Welcome_Model->get_states();
				...
				//for serach slider
					...
					$template['specialtiesslider'] =$this->Home_Model->get_specialties();
					$template['insuranceslider'] =$this->Home_Model->get_insurance();
					$template['page'] = "Welcome_hospital";					
			}		
				$template['page_title'] = "Welcome Page";
				$template['data'] = "Welcome page";
...
//pdf for medical center dashboard
...
public function buy_package($id=null){
	if ($this->session->userdata('super_person') == 1){
        ...
        //$userID = $product['customer_id']; //current user id
