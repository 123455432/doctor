...
class Doctor_Model extends CI_Model
	...
	function _construct()
		...
		public function get_singlegalpic($id){
			$this->db->select("*");
		...
		public function getall_doctorscalendarnavi($doc_id,$status,$dateCnt,$columns =3){
		...
		$prevcolumns = 0;
		//get all days as variable array
		...
		$query = $result->row(); 
            ...
            // $res_data[$value->id]['name'] = (!empty($value->doctor_firstname) || !empty($value->doctor_lastname)) ? $value->doctor_firstname."&nbsp".$value->doctor_lastname : 'Not specified';
 			//$res_data[$value->id]['avg_rating'] = (!empty($value->avg_rating)) ? $value->avg_rating : '0';
		//	$res_data[$value->id]['address'] = (!empty($value->city_name) || !empty($value->state_name) || !empty($value->country_name) || !empty($value->doctor_office_zip)) ? $value->city_name.'<br>'.$value->state_name.' '.$value->country_name.'<br>'.$value->doctor_office_zip : 'Not Specified'; 				 								
			//$res_data[$value->id]['display_image'] = (!empty($value->display_image)) ? $value->display_image : $s."assets/frontend/img/sch-profpic1.png";
		...
		if(! empty($query->vacation_time)){
            ...
            foreach($array1 as $keey=>$valus){
				...
				//array_push($res_data[$valus->doctor_id]['apntdate'],$keey);
                //$res_data[$valus->doctor_id]['apnttime'][$keey] =$valus;
			   /*$starttime =$valus->start_time;
			   $endtime=$valus->end_time;			   
			   $r=$this->get_apnt_times( $starttime,$endtime );
			   
                $res_data[$valus->doctor_id]['apnttime'][$valus->appointment_date][]=$starttime;*/
				...
				foreach ($res_data as $key => $value) {					
					if($value['working_time'] != 'none'){
						foreach ($days as  $day) {
							...
							//$res_data[$key]['vacation_time'] = (array_key_exists('vacation_time',$value)) ? $this->get_vacation_time($value['vacation_time'],$data[$key]['vacation_date']) : '';
						}
					}
				}
				//echo "<pre>";
			...
			$data_res ='';
				...
				foreach ($res_data as $key => $value) {					
					...
					// $data_res .='<div class="evnt-mn doctor">';
					//$data_res .='<div class="evt-br-doc" id="evt-br-doc_'.$key.'">';
					//$data_res .=$this->profile_html($value,$s,$key,0);
					//$data_res .='</div>';		
					//$data_res .= '<input type="hidden" class="currentDate_'.$key.'"  value="'.$date.'">';
					$data_res .='
					<div class="date-head">
				    <div class="previouscalapp" id="'.$key.'" data-date="'.$Day.' '.$date.'" data-selected="true" data-div="evt-br-doc_'.$key.'">
					...
					<div class="nextcalapp " id="'.$key.'" data-date="'.$Day.' '.$date.'"  data-selected="true" data-div="evt-br-doc_'.$key.'">
   ...
   //post filtered data 
   //set status to be NULL and datecnt & column are 0,3
   public function get_freshcal($doc_id,$status=0,$dateCnt=0,$columns =3){	  	
		$currDate = date('Y-m-d');			
		$days=array('mon','tue','wed','thu','fri','sat','sun');		
        if($status=='next'){		
		$date= ($dateCnt==0) ? date('Y-m-d') : date('Y-m-d' , strtotime($dateCnt. ' + '.$columns.' days'));
            ...
            foreach($array1 as $keey=>$valus){
 			...
 			//} 	
				...
				if(!empty ($value['working_time'])){	
					if($value['working_time'] != 'none'){
						//echo '<pre>';print_r($value['working_time']['mon']);die;
						foreach ($days as  $day) {
		...
		public function getall_doctorscalendar($data,$status = null,$dateCnt=0,$columns =3){
			//define current date 
		$currDate = date('Y-m-d');	
		//get all days as variable array
		$days=array('mon','tue','wed','thu','fri','sat','sun');
		...
		if(isset($data['gender']) and !empty($data['gender'])) {	
           $gender = $data['gender'];	
        $this->db->where("do.doctor_sex LIKE '%$gender%'");	
		}			
		$this->db->group_by('do.id');
		$this->db->order_by('do.id','desc');
		$this->db->limit(4);
		...
		if(! empty($value->vacation_time)){
			...
			$data_res ='';
				...
				foreach ($res_data as $key => $value) {	
				//$classunique="evt-br-doc_".$query['id'];
					// $data_res .='<div class="row">';
					// $data_res .='<div class="evnt-mn doctor">';
		...
		public  function get_doctor_details($id,$second_date,$date){
	...
	function get_times($startdate,$enddate){
		...
		public function get_latlang_doctor($id){
...
public function getData($loadType,$loadId){
	...
	public function getDataReason($loadType,$loadId){
		...
		public function get_singlereview($id)
		{
			...
			//$this->db->group_by('rate.patient_id');
		...
		public function update_bklevelone($id,$data){
			$this->db->where('id', $id);
		...
		public function getall_doctorscalendarnavianother($doc_id,$status,$dateCnt,$columns =3){
