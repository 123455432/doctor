...
{		
	...
	public function get_singleclinic($id){
		...
		public function get_latlang_clinic($id){
			
			$this->db->select('display_image,clinic_name,latitude,longitude');
			$this->db->from('clinic');
			$this->db->where('id', $id);
			$query = $this -> db -> get();
			return $query->row();
		}
		public function get_clinicdoctors($id){
		...
		//$clinic_id='do.clinic_id';
		//$subclinic_id=explode(",",$clinic_id);
       ...
       // $arraycldoc = array('do.clinic_id' => $id,'status' => '1');
        //$this->db->where($arraycldoc); 		
	   ...
	   $this->db->where("(do.trial_date >='$currentDate' OR do.end_date >='$currentDate')");
		...
		public function get_singleclinicgallery($id){
